...
from app.images import imagekit, IMAGEKIT_PRIVATE_KEY, IMAGEKIT_PUBLIC_KEY
import shutil
import os
import uuid
import tempfile
...
class PostMetadataCreate(BaseModel):
    caption: str = ""
    url: str
    file_type: str
    file_name: str
...
async def get_upload_auth(user: User = Depends(current_active_user)):
    if not IMAGEKIT_PRIVATE_KEY or not IMAGEKIT_PUBLIC_KEY:
        raise HTTPException(status_code=500, detail="ImageKit credentials are not configured.")

    token = uuid.uuid4().hex
    expire = int(time.time()) + 60 * 10
    signature_payload = f"{token}{expire}".encode()
    signature = hmac.new(
        IMAGEKIT_PRIVATE_KEY.encode(), signature_payload, hashlib.sha1
    ).hexdigest()
...
@app.post("/posts")
async def create_post_from_metadata(
    payload: PostMetadataCreate,
    user: User = Depends(current_active_user),
    session: AsyncSession = Depends(get_async_session),
):
    if payload.file_type not in {"image", "video"}:
        raise HTTPException(status_code=400, detail="file_type must be either 'image' or 'video'.")
